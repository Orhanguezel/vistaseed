...
- [x] P1.4 MVP (`POST /api/v1/offers/public`, `source` `toplu-satis` / `teklif-al`; `/{locale}/toplu-satis` + mevcut `teklif-al` formu)
...
| Bayi ağı | `GET /api/v1/dealers/public`; `dealer_profiles` + `136` konum alanları; `/{locale}/bayi-agi`. |
| Toplu satış | `POST /api/v1/offers/public` (`source`: `toplu-satis`, `teklif-al`); `/{locale}/toplu-satis` (EN `/bulk-sales`, DE `/grossverkauf`). |
