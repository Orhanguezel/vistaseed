...
export async function repoListEmailTemplates(params: { q?: string; limit: number; offset: number }) {
  let qb = db.select().from(emailTemplates).$dynamic();
  if (params.q) qb = qb.where(like(emailTemplates.template_key, `%${params.q}%`));
