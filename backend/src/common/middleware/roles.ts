...
export type AppRole = 'admin' | 'carrier' | 'customer';
...
/** Taşıyıcı veya admin erişimine izin verir. */
export async function requireCarrierOrAdmin(req: FastifyRequest, reply: FastifyReply) {
  return requireRoles(req, reply, ['admin', 'carrier']);
}
