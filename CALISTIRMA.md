# vistaseeds — Projeyi çalıştırma

Bu dosya `projects/vistaseeds` kökü içindir. vistaseeds, **Tarım Dijital Ekosistem** monoreposunda `@agro/shared-backend` ile bağlıdır; ilk kurulumda ortak paket derlemesi gerekir.
...
Bu repoda backend, monorepo kökündeki `packages/shared-backend` derlemesini kullanır. `build:shared` hem ekosistem kökünde hem `projects/vistaseeds/package.json` içinde tanımlıdır; ikisi de aynı `../../packages/shared-backend` hedefine gider.
...
**B — vistaseeds kökünden** (`projects/vistaseeds`; bu dosyanın bulunduğu klasör):

```bash
cd ~/Documents/Projeler/tarim-dijital-ekosistem/projects/vistaseeds
bun run build:shared
```

Bu komut `../../packages/shared-backend` yoluna (monorepo kökündeki tek ortak `packages/`) gidip TypeScript derlemesini çalıştırır. vistaseeds altında ayrı bir `packages/` klasörü yoktur; tüm projeler `tarim-dijital-ekosistem/packages/` örneğini paylaşır.
...
```bash
cd projects/vistaseeds/backend
cp .env.example .env
...
```bash
cd projects/vistaseeds/frontend
...
Harici sitelerden içe RSS almak için ayrı yapılandırma (kaynak URL listesi) ileride `site_settings` veya env ile eklenecek; şu an vistaseeds blogu dışa RSS verir.
