...
export default async function PublicLayout({
  ...
  const organizationSchema = {
    ...
    url: SITE_URL,
    ...(settings.site_logo && { logo: settings.site_logo.startsWith("http") ? settings.site_logo : `${SITE_URL}${settings.site_logo}` }),
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <Footer
        ...
        helpDescription={
          <>
            <a href={`/${currentLocale}${ROUTES.static.faq}`} className="text-white/70 underline hover:text-white">{tFooter("faqLink")}</a>{" "}
            {tFooter("helpDescriptionMiddle")}{" "}
            <a href={`/${currentLocale}${ROUTES.static.contact}`} className="text-white/70 underline hover:text-white">{tFooter("contactLink")}</a>.
          </>
        }
