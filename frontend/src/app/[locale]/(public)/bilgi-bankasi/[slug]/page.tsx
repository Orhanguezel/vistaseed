...
export default async function LibraryDetailPage({ params }: LibraryDetailPageProps) {
  ...
  const breadcrumbLd = buildBreadcrumbJsonLd([
    ...
    { name: t("breadcrumbKnowledge"), url: `${origin}${toLocalizedPath("/bilgi-bankasi", locale)}` },
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="flex items-center gap-3 mb-6">
            <span className="text-[11px] font-black uppercase tracking-widest text-brand bg-brand/10 px-3 py-1 rounded-full backdrop-blur-sm">
              {item.type === "support" ? t("supportBadge") : t("knowledgeBadge")}
            </span>
            {item.published_at && (
              <span className="text-[11px] font-bold text-muted uppercase tracking-wider text-muted-foreground/60">
                {t("publishedLabel")}: {new Date(item.published_at).toLocaleDateString(locale)}
              </span>
            )}
          </div>

          <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-foreground mb-6">{item.title}</h1>

          {item.summary && (
            <p className="text-lg leading-relaxed max-w-2xl font-medium text-foreground/70">{item.summary}</p>
          )}
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2">
            {fullImg && (
              <div className="relative aspect-video rounded-3xl overflow-hidden mb-12 shadow-2xl border border-border/10 group">
                <img
                  src={fullImg}
                  alt={item.title}
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-navy-mid p-8 rounded-3xl border border-white/5 shadow-2xl">
              <h4 className="text-white text-xs font-black uppercase tracking-widest mb-4">{t("helpTitle")}</h4>
              <p className="text-white/60 text-sm leading-relaxed mb-6">{t("helpDescription")}</p>
              <Link
                href={toLocalizedPath("/iletisim", locale)}
                className="inline-block px-5 py-2.5 bg-brand text-white text-xs font-black uppercase tracking-widest rounded-full hover:bg-brand-dark transition-colors"
              >
                {t("contactCta")}
              </Link>
            </div>
            ...
            <Link
              href={toLocalizedPath("/bilgi-bankasi", locale)}
