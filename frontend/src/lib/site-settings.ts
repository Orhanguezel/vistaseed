...
export async function fetchSiteSettings(locale = "tr"): Promise<SiteSettings> {
  ...
  try {
    ...
    const res = await fetch(
      `${API_URL}/api/site_settings?key_in=${keys}&locale=${locale}`,
      { next: { revalidate: 300 } },
    );
...
export async function fetchHomepageSettings(locale = "tr"): Promise<HomepageData> {
  try {
    const res = await fetch(
      `${API_URL}/api/site_settings/homepage?locale=${locale}`,
      { next: { revalidate: 300 } },
    );
...
export async function fetchAboutPageData(locale = "tr"): Promise<AboutPageData | null> {
  const fetchLocale = async (targetLocale: string) => {
    const res = await fetch(`${API_URL}/api/site_settings/about_page?locale=${encodeURIComponent(targetLocale)}`, {
      next: { revalidate: 300 },
    });
...
async function fetchSettingValue<T>(key: string, locale = "tr"): Promise<T | null> {
  try {
    const res = await fetch(`${API_URL}/api/site_settings/${key}?locale=${locale}`, {
      next: { revalidate: 300 },
    });
    if (!res.ok) return null;
    const row = await res.json();
...
export async function fetchCustomPageBySlug(slug: string, locale = "tr"): Promise<CustomPageData | null> {
  try {
    const res = await fetch(`${API_URL}/api/custom-pages/by-slug/${slug}?locale=${locale}`, {
      next: { revalidate: 300 },
    });
    if (!res.ok) return null;
    return res.json();
